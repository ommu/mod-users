...
class SettingController extends Controller
{
	...
	public function accessRules() 
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('manage','add','edit','runaction','delete','publish','headline','manual'),
				'users'=>array('@'),
	...
	public function actionIndex() 
	{
		...
		$setting = UserSetting::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('UserSetting', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
		...
		$this->pageTitle = Yii::t('phrase', 'User Settings');
	...
	public function actionView($id) 
	{
		...
		$this->pageTitle = Yii::t('phrase', 'View User Settings');
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['UserSetting'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-user-setting',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'User Settings success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'UserSetting success created.'));
		...
		$this->pageTitle = Yii::t('phrase', 'Create User Settings');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['UserSetting'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'UserSetting success updated.'));
					//$this->redirect(array('view','id'=>$model->id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update User Settings');
		$this->pageDescription = '';
		$this->pageMeta = '';
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if($model->delete()) {
				echo CJSON::encode(array(
					...
					'get' => Yii::app()->controller->createUrl('manage'),
					'id' => 'partial-user-setting',
					'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'User Settings success deleted.').'</strong></div>',
				));
			}
			...
			$this->pageTitle = Yii::t('phrase', 'Delete User Settings');
			$this->pageDescription = '';
			$this->pageMeta = '';
