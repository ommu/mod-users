...
class VerifyController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$setting = UserVerify::model()->findByPk(1,array(
		...
		$dataProvider = new CActiveDataProvider('UserVerify', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
	...
	public function actionAdd() 
	{
		...
		if(isset($_POST['UserVerify'])) {
				...
				if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'User Verifies success created.').'</strong></div>',
						));
					} else {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'UserVerify success created.'));
					//$this->redirect(array('view','id'=>$model->verify_id));
					$this->redirect(array('manage'));
				}
			}
		}
		...
		$this->pageTitle = Yii::t('phrase', 'Create User Verifies');
		$this->pageDescription = '';
		$this->pageMeta = '';
	...
	public function actionEdit($id) 
	{
		...
		if(isset($_POST['UserVerify'])) {
			...
			*/
			...
			if(isset($_GET['enablesave']) && $_GET['enablesave'] == 1) {
				if($model->save()) {
					Yii::app()->user->setFlash('success', Yii::t('phrase', 'UserVerify success updated.'));
					//$this->redirect(array('view','id'=>$model->verify_id));
					$this->redirect(array('manage'));
				}
			}
		...
		$this->pageTitle = Yii::t('phrase', 'Update User Verifies');
		$this->pageDescription = '';
		$this->pageMeta = '';
