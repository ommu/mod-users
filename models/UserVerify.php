...
{
	public $gridForbiddenColumn = ['modified_date', 'modified_search', 'deleted_date', 'verify_ip', 'expired_date'];
	...
	public function rules()
	{
		return [
			...
			[['user_id', 'code', 'verify_ip', 'modified_id'], 'required'],
			[['verify_date', 'expired_date', 'modified_date', 'deleted_date'], 'safe'],
			[['code'], 'string', 'max' => 64],
	...
	public function init() 
	{
		...
		$this->templateColumns['verify_date'] = [
			...
			'value' => function($model, $key, $index, $column) {
				return !in_array($model->verify_date, ['0000-00-00 00:00:00','1970-01-01 00:00:00']) ? Yii::$app->formatter->format($model->verify_date, 'datetime') : '-';
			},
			'format'	=> 'html',
		];
		$this->templateColumns['verify_ip'] = 'verify_ip';
	...
	public function beforeValidate() 
	{
		if(parent::beforeValidate()) {
			if($this->isNewRecord) {
				...
				$this->verify_ip = Yii::$app->request->userIP;
	...
	public function afterSave($insert, $changedAttributes) 
	{
		...
		// send email fuinction after verify
	}
}
